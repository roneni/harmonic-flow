...
// ── Brushed silver aluminum — studio panel face ───────────────────────────
const SILVER_METAL = [
  "repeating-linear-gradient(0deg, transparent 0px, transparent 3px, rgba(0,0,0,0.045) 3px, rgba(0,0,0,0.045) 4px)",
  "linear-gradient(180deg, #d8d8d8 0%, #bcbcbc 22%, #c8c8c8 50%, #b4b4b4 78%, #d0d0d0 100%)",
].join(", ");
...
function PhillipsScrew({ uid }: { uid: string }) {
  return (
    <svg width="22" height="22" viewBox="0 0 20 20" aria-hidden="true">
      ...
      <circle cx="10" cy="10.8" r="9"   fill="rgba(0,0,0,0.50)" />
      <circle cx="10" cy="10"   r="9"   fill="#0e0e0e" stroke="#1e1e1e" strokeWidth="0.6" />
...
// ── Ring geometry ──────────────────────────────────────────────────────────
const VU_PCT  = 65;
const VU_OFF  = (100 - VU_PCT) / 2;  // 17.5%
const BOLT_R  = 44;
const BOLT_SZ = 3.6;
...
export function StudioInputUpload() {
  ...
  const ringGlow =
    panelState === "idle"
      ? "0 0 18px rgba(132,204,22,0.25), 0 0 40px rgba(132,204,22,0.08)"
      : panelState === "processing"
        ? "0 0 28px rgba(132,204,22,0.70), 0 0 56px rgba(132,204,22,0.32)"
        : ledOn
          ? "0 0 40px rgba(132,204,22,0.95), 0 0 80px rgba(132,204,22,0.45)"
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          maxWidth:      1280,
          paddingLeft:   "clamp(28px, 3.5vw, 52px)",
          paddingRight:  "clamp(28px, 3.5vw, 52px)",
          paddingTop:    "clamp(20px, 2.5vw, 36px)",
          paddingBottom: "clamp(20px, 2.5vw, 36px)",
        }}
      >
        {/* ── Corner screws ────────────────────────────────────────────── */}
        <span style={{ position: "absolute", top: 12,  left:  20 }}><PhillipsScrew uid="tl" /></span>
        <span style={{ position: "absolute", top: 12,  right: 20 }}><PhillipsScrew uid="tr" /></span>
        <span style={{ position: "absolute", bottom: 12, left:  20 }}><PhillipsScrew uid="bl" /></span>
        <span style={{ position: "absolute", bottom: 12, right: 20 }}><PhillipsScrew uid="br" /></span>

        {/*
          Column layout: header sits DIRECTLY above the ring so both share
          the same horizontal centre. Signal Input is absolutely positioned
          to the ring's right so it doesn't disturb the flex-centre.
        */}
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>

          {/* ── Header — centred over ring ──────────────────────────── */}
          <p style={{
            margin:        0,
            ...
            textShadow:    "0 1px 0 rgba(255,255,255,0.65), 0 -1px 0 rgba(0,0,0,0.12)",
          }}>
            Studio Input Upload
          </p>

          {/* ── Ring row — ring + absolutely-placed signal indicator ─── */}
          <div style={{ position: "relative" }}>
            ...
            <div style={{
              ...
              borderRadius: "50%",
              background: [
                "radial-gradient(circle at 38% 33%, rgba(80,80,80,0.45) 0%, rgba(0,0,0,0) 48%)",
                "repeating-conic-gradient(from 0deg, #161616 0deg, #2c2c2c 5deg, #161616 10deg)",
              ].join(", "),
              boxShadow: [
              ...
              <div
                ...
                style={{
                  ...
                  top:            `${VU_OFF}%`,
                  borderRadius:   "50%",
                  background:     "radial-gradient(circle at 44% 38%, #1c1c1c 0%, #0c0c0c 45%, #040404 100%)",
              ...
              >
                ...
                {/* Flat line — idle */}
                {panelState === "idle" && (
                  <div style={{ width: "38%", height: 1, background: "#282828", marginBottom: 12, flexShrink: 0 }} />
                )}

                {/* State text — white on dark circle */}
                <div style={{ textAlign: "center", padding: "0 14%", zIndex: 1 }}>
                  {panelState === "idle" && (
                    <>
                      ...
                      <div style={{ color: "#909090", fontSize: "clamp(7px, 0.55vw, 9px)", letterSpacing: "0.07em", textTransform: "uppercase", fontFamily: "var(--font-mono)", lineHeight: 1.75 }}>
            ...
            {/* ── SIGNAL INPUT — absolute right of ring ─────────────────── */}
            <div style={{
              position:   "absolute",
              left:       "calc(100% + clamp(20px, 2.5vw, 44px))",
              top:        "50%",
              ...
              <div style={{
                ...
                background:   signalActive ? "#84cc16" : "#2a2a2a",
          ...
          <div style={{
            ...
            <div style={{
              ...
              borderRadius: "50%",
              border:       "1px solid #4a4a4a",
              background:   panelState === "processing" ? "#84cc16" : "#2a2a2a",
      ...
      <div style={{
        height:    7,
        background: "linear-gradient(0deg, #1c1c1c, #2e2e2e)",
